...
# -*- coding: utf-8 -*-
"""
Three-phase Raster + Population Rate (NaN-aware, per-trial coverage)
"""
...
def analyze_file(fname, window, align_to_list, label, output_dir):
    ...
    plot_raster_and_rate(all_spikes_abs, align_times, neuron_map, fname, label, window, output_dir)
...
def main():
    ...
    for f in files:
        analyze_file(f, window=(-0.5, 0), align_to_list=[STIM_ON_ID], label="Pre Stim", output_dir=output_dir)
        analyze_file(f, window=(0, 0.5), align_to_list=[STIM_ON_ID], label="During Stim", output_dir=output_dir)
        analyze_file(f, window=(0, 0.5),
                     align_to_list=[STIM_OFF_KEEP_ID, STIM_OFF_BROKEN_ID, SACCADE_CHOICE_ON_ID],
                     label="Post Stim", output_dir=output_dir)
