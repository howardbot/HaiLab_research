...
def plot_neuron_raster(all_spikes, stim_on_times, neuron_map, fname):
    ...
    for neuron_idx in range(num_neurons):
        ...
        for trial_idx in range(num_trials):
            ...
            aligned = flattened.astype(float) - stim_on_times[trial_idx]
            #show within 500 ms
            trial_spikes = aligned[(aligned >= 0) & (aligned <= 0.5)]
            all_spike_times.extend(trial_spikes)

        if all_spike_times:
            plt.vlines(all_spike_times, neuron_idx + 0.5, neuron_idx + 1.5, color=color, linewidth=0.7)

    plt.title(f"Neuron-Centric Raster Plot (Trials Pooled) - {fname}")
    plt.xlabel("Time (s) from stim onset")
    plt.ylabel("Neuron Index")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"raster_neuronwise_{fname}.png"))
    plt.close()
...
def analyze_file(fname):
    ...
    plot_neuron_raster(all_spikes, stim_on_times, neuron_map, fname)
